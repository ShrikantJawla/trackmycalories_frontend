...
export const categoryWiseProductDisplay = () => async (dispatch) => {
    try {
        const { data } = await axios.get(`${baseURL}${process.env.REACT_APP_CATEGORY_WISE_PRODUCT_DISPLAY}`);
...
export const getSingleShopProductReviews = (id, q) => async (dispatch) => {
    try {
        const { data } = await axios.get(`${baseURL}${process.env.REACT_APP_SINGLE_PRODUCT_REVIEWS_BY_ID}/${id}?page=${q}`, {
            headers: {
...
export const getSingleShopProduct = (id) => async (dispatch) => {
    try {
        const { data } = await axios.get(`${baseURL}${process.env.REACT_APP_SINGLE_PRODUCT_BY_ID}/${id}`)
...
export const addProductReview = (data, productId) => async (dispatch) => {
    try {
        await axios.post('http://localhost:8080/shop/product/reviews/add-review', data, {
            headers: {
                token: localStorage.getItem('checkmycalorieToken')
            }
        });
...
export const addProductLikesOrDislike = (typeOfQuery, reviewId, productId) => async (dispatch) => {
    ...
    try {
        if (typeOfQuery === 'like') {
            url = `http://localhost:8080/shop/product/reviews/update-likes/${reviewId}`
        } else {
            url = `http://localhost:8080/shop/product/reviews/update-disLikes/${reviewId}`
        }
...
export const getShopProductsByQuery = (allCategory, page, sort) => async (dispatch) => {
    try {
        const { data } = await axios.get(`http://localhost:8080/shop/products?category=${allCategory}&page=${page}&sort=${sort}`)
...
export const getCategoriesWithLengthForFilter = () => async (dispatch) => {
    try {
        const { data } = await axios.get(`http://localhost:8080/shop/products/categoriesAndLength`)
